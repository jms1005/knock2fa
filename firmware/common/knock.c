 ...
 * 간격 벡터가 통째로 무너진다.
 * 실험 (1, 2026-09-12): 50ms 로는 매 타격마다 잔향이 한 번씩 더 잡혔다
 * (UART 로그 g0~g2 에서 재현). 50/100/200/400ms 순서로 올려봤지만, 값을
 * 얼마로 두든 그 값과 거의 같은 간격에서 잔향이 다시 잡혔다 — 즉 이 피에조
 * (부저용 디스크)가 250ms 가 지나도 여전히 문턱을 넘나들 만큼 오래 운다는
 * 뜻이다. 브레드보드 대신 책상처럼 단단한 면에 고정하고 디스크에 말랑한
 * 재질(블루택 등)로 댐핑을 추가하면 크게 줄지만 완전히 없어지진 않는다.
 * 250ms 를 브링업 기준값으로 확정한다 — 초당 4회까지 허용하므로 사람이
 * 두드리는 속도에는 충분하다. 잔향의 완전한 억제(FRR/FAR 곡선 기반 최종
 * 값)는 실험 1에서 마저 확정할 것.
 */
#define BLANK_TICKS   (250UL  * KNOCKFP_TICKS_PER_MS)
